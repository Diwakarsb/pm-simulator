  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="text-xs text-muted uppercase tracking-wide">Funnel</div>
        ...
        {config.steps.map((step) => {
          ...
          return (
            <div key={step.name} className="flex-1 flex flex-col items-center justify-end h-full">
              <div className="text-[11px] font-mono mb-1">{step.count.toLocaleString()}</div>
              <div
      ...
      <div className="flex gap-2 mt-2">
        {config.steps.map((step) => (
          <div key={step.name} className="flex-1 text-center">
            <div className="text-[11px] text-muted truncate">{step.name}</div>
